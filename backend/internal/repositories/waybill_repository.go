...
func (r *waybillRepository) GetWaybillByID(ctx context.Context, waybillID int64) (models.WaybillDetail, error) {
	...
	query := `
		SELECT w.id, w.waybill_number, w.issue_date, COALESCE(w.dispatch_date, w.issue_date), w.expected_delivery_date,
		 w.actual_delivery_date, w.status, w.sender_id,
			COALESCE(c1.name, ''), COALESCE(c1.phone_mobile, ''), COALESCE(c1.phone_fixed, ''),
			w.receiver_id, COALESCE(c2.name, ''), COALESCE(c2.phone_mobile, ''), COALESCE(c2.phone_fixed, ''),
			COALESCE(w.driver_id, 0), COALESCE(d.first_name || ' ' || d.last_name, ''),
			COALESCE(d.phone, ''), COALESCE(d.license_number, ''), COALESCE(d.national_code, ''),
			COALESCE(w.vehicle_id, 0), COALESCE(v.model, ''), COALESCE(v.plate, ''),
			COALESCE(w.origin_location_id, 0), COALESCE(l1.name, ''),
			COALESCE(w.destination_location_id, 0), COALESCE(l2.name, ''), w.total_weight,
	...
	err := r.db.QueryRowContext(ctx, query, waybillID).Scan(
		...
		&res.Receiver, &res.ReceiverPhoneMobile, &res.ReceiverPhoneFixed,
		&res.DriverID, &res.Driver, &res.DriverPhoneNum, &res.DriverLicenseNum, &res.DriverNationalCode,
		&res.VehicleID, &res.Vehicle, &res.VehiclePlate,
